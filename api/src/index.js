...
import { tickDisputeClocks, syncMailStatuses } from './services/dispute.service.js';
...
const tick = async () => {
  try { await runAlerts(); await expireCancelled(); await syncMonitoringScores(); await tickDisputeClocks(); await syncMailStatuses(); } catch (err) { console.error('housekeeping failed:', err.message); }
};
